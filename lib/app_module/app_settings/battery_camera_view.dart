...
class BatteryCameraView extends StatelessWidget {
  const BatteryCameraView({Key? key}) : super(key: key);
      ...
      appBar: AppBar(
        ...
        title: Obx(() => AppText(
          text: controller.selectedTileIndex.value != -1
              ? "${MyStrings.batterycamerafaqs.tr}"
              : "${MyStrings.batterycamera.tr}",
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _settingsTile(controller, 5, "${MyStrings.audibleandvisualalarm.tr}"),
        ],
      ),
    );
  }
  ...
  Widget _buildDetailList(AppSettingController controller) {
    final items = controller.batterycameratileItems[controller.selectedTileIndex.value];
