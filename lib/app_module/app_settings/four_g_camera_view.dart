...
class FourGCameraView extends StatelessWidget {
  const FourGCameraView({Key? key}) : super(key: key);
      ...
      appBar: AppBar(
        ...
        title: Obx(() => AppText(
          text: controller.selectedTileIndex.value != -1
              ? "${MyStrings.fourgcamerafaqs.tr}"
              : "${MyStrings.fourgcamera.tr}",
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _settingsTile(controller, 3, "${MyStrings.trafficpurchase.tr}"),
          _settingsTile(controller, 4, "${MyStrings.hardwareissues.tr}"),
          _settingsTile(controller, 5, "${MyStrings.localplayback.tr}"),
        ],
      ),
    );
  }
  ...
  Widget _buildDetailList(AppSettingController controller) {
    final items = controller.fourgcameratileItems[controller.selectedTileIndex.value];
